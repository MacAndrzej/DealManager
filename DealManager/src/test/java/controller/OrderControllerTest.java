...
public class OrderControllerTest {
	...
	public void setup() {
		...
		mockMvc = MockMvcBuilders.standaloneSetup(orderController).setControllerAdvice(new ControllerExceptionHandler())
				.build();
	}
	...
	public void testListDeals_ListIsNotEmpty() throws Exception {
		...
		mockMvc.perform(get("/order/list")).andExpect(status().isOk()).andExpect(view().name("listAllOrders"))
				.andExpect(status().is(200)).andExpect(model().size(1)).andExpect(model().attributeExists("deals"))
				.andExpect(model().attribute("deals", expectedDeal)).andDo(print());
	...
	public void testListActiveDeals_ListIsNotNull() throws Exception {
		...
		mockMvc.perform(get("/order/listActive")).andExpect(status().isOk()).andExpect(view().name("listActiveOrders"))
				.andExpect(status().is(200)).andExpect(model().size(1)).andExpect(model().attributeExists("deals"))
				.andExpect(model().attribute("deals", expectedDeal)).andDo(print());
	...
	public void testShowFormForUpdateOrder_EntryExists() throws Exception {
		...
		mockMvc.perform(get("/order/showFormForUpdateOrder?dealId=1")).andExpect(view().name("dealForm"))
	...
	public void testShowFormForUpdateOrder_EntryDoesNotExist() throws Exception {
		...
		mockMvc.perform(get("/order/showFormForUpdateOrder?dealId=1")).andExpect(status().isNotFound())
	...
	public void testShowFormForUpdateOrder_EntryExist_StringInsteadOfLong() throws Exception {
		...
		mockMvc.perform(get("/order/showFormForUpdateOrder?dealId=kk")).andExpect(status().isBadRequest())
	...
	public void testDisableOrder_WhenEntryExists() throws Exception {
		...
		mockMvc.perform(get("/order/disableOrder?dealId=1")).andExpect(redirectedUrl("/order/listActive"))
				.andDo(print());
	...
	public void testDisableOrder_WhenEntryDoesNotExist() throws Exception, IdNotFoundException {
		...
		mockMvc.perform(get("/order/disableOrder?dealId=1")).andExpect(status().isNotFound())
				.andExpect(view().name("404")).andDo(print());
	...
	public void testDisableOrder_EntryExist_StringInsteadOfLong() throws Exception, IdNotFoundException {
		...
		mockMvc.perform(get("/order/disableOrder?dealId=gg")).andExpect(status().isBadRequest())
				.andExpect(view().name("400"));
	}
	...
	public void showFormForAddOrder() throws Exception {
		...
		mockMvc.perform(get("/order/showFormForAddOrder")).andExpect(view().name("dealForm"))
				.andExpect(model().attributeExists("order")).andExpect(model().attributeExists("allSystems"))
				.andExpect(model().size(2)).andExpect(status().is(200));
	}

	@Test
	public void testAddDeal_NoErrorsAfterValidation() throws Exception {

		mockMvc.perform(post("/order/saveOrder").param("orderNumber", "1/2015").param("amount", "3000")
				.param("amountPeriod", "MONTH").param("active", "1"))
	...
	public void testAddDeal_ValidationErrors() throws Exception {
		...
		mockMvc.perform(post("/order/saveOrder").param("orderNumber", "1/2015").param("amount", "3000")
				.param("amountPeriod", "MONTH").param("active", "1").param("id", "bike"))
	...
	public void importCsvTxtOrder() throws Exception {
		...
		mockMvc.perform(get("/order/showImportOrdersForm")).andExpect(view().name("importOrders"));
	}
